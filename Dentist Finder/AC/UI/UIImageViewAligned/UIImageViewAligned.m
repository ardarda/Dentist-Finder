...
- (void)layoutSubviews
{
    ...
    CGFloat quarterValue = realsize.width/4;
//    DDLogDebug(@"quarterValue:%f", quarterValue);
    // Start centered
    ...
    //ATA Specific
    // |---|2---|---|4---|
    if ((_alignment & UIImageViewAlignmentMaskInnerLeft) != 0){
        realframe.origin.y = 0;
        CGFloat testX = -quarterValue + screenSize.width / 2;
        if ((realsize.width + testX) < screenSize.width) testX = 0;
        realframe.origin.x = testX;
    }
    else if ((_alignment & UIImageViewAlignmentMaskInnerRight) != 0){
        realframe.origin.y = 0;
        CGFloat testX = -3 * quarterValue + screenSize.width / 2;
        if ((realsize.width + testX) < screenSize.width) testX = CGRectGetMaxX(self.bounds) - realframe.size.width;
        realframe.origin.x = testX;
...
- (CGSize)realContentSize
{
    ...
    switch (self.contentMode)
    {
        ...
        {
            //Workaround for preventing oversize in animations @arda
            CGSize fullSize = self.bounds.size;
            if (fullSize.height > [UIScreen mainScreen].bounds.size.height) fullSize.height = [UIScreen mainScreen].bounds.size.height;
            //
            
            float scalex = fullSize.width / _realImageView.image.size.width;
            float scaley = fullSize.height / _realImageView.image.size.height;
...
- (BOOL)alignLeft2
{
    return (_alignment & UIImageViewAlignmentMaskInnerLeft) != 0;
}
- (void)setAlignLeft2:(BOOL)alignLeft2
{
    if (alignLeft2)
        self.alignment |= UIImageViewAlignmentMaskInnerLeft;
    else
        self.alignment &= ~UIImageViewAlignmentMaskInnerLeft;
}
...
- (BOOL)alignRight4
{
    return (_alignment & UIImageViewAlignmentMaskInnerRight) != 0;
}

- (void)setAlignRight4:(BOOL)alignRight4
{
    if (alignRight4)
        self.alignment |= UIImageViewAlignmentMaskInnerRight;
    else
        self.alignment &= ~UIImageViewAlignmentMaskInnerRight;
